...
### 4.7 Выбор ru_authors кандидатов (MetadataExtractor, `_pick_ru_authors`)

Данный подраздел фиксирует логику отбора кандидатов в `ru_authors` anchors на уровне MetadataExtractor.

**Negative gate (отклонение):**
- Кандидат отклоняется если `is_running_header(text)` = True (running headers вида «Том 21, № 3»)

**Positive gate (принятие) — достаточно одного из:**
- `looks_like_author_byline(text)` — standard 2-initial byline: «Фамилия И.О.» или «Фамилия И.О., ...»
- `looks_like_single_initial_byline(text)` — single-initial RU byline: «Фамилия И.»
- `extract_single_initial_byline_prefix(text)` — если кандидат является merged block («Гелприн М. Рассказ Майка...»): функция извлекает только prefix byline («Гелприн М.»); MetadataExtractor эмитирует `{**candidate, "text": prefix}`, не полный merged block

**Семантический инвариант:** `_pick_ru_authors` изменяет только то, КАКИЕ text_blocks становятся `ru_authors` anchors. Алгоритм извлечения фамилии STEP A→B→C в MetadataVerifier (§4.1) не изменён. Canonical naming semantics неизменна: surname-based filename определяется корректным upstream author extraction, а не post-hoc special-case substitution.

**EN-path exclusion (намеренное):**
- `_pick_en_authors` НЕ использует single-initial gate
- Причина: при разработке fix'а для Mh_2026-03 обнаружен empirical false-positive — строка «Lana R. Dzik;» в Western-format корреспондентской секции (формат «FirstName MI. LastName») принималась за single-initial byline. MetadataVerifier STEP A возвращал «Lana» вместо «Zaklyazminskaya» для Mg_2025-12 p67-73.
- Guardrail `TOP_REGION_FRAC=0.40` в `_pick_en_authors` (вертикальное ограничение: принимаются только кандидаты из верхних 40% страницы) сохраняется как самостоятельная защитная мера от библиографических ссылок снизу страницы.
- Разрешение single-initial matching в EN-path требует отдельного исследования и явного version bump политики.
...
**Mh_2026-03 (acceptance fixture для v_1_3 — single-initial byline support):**
- Журнал: «Психическое здоровье»
- Статей: **9**, T=L=E=9, sha256 verified
- Исправленные filenames (результат fix'а):
  - `Mh_2026-03_033-039_Vasilkova.pdf` (было: `Analiziruya.pdf` — running header в ru_authors)
  - `Mh_2026-03_055-066_Plotkin.pdf` (было: `Skim.pdf` — фрагмент тела статьи в ru_authors)
  - `Mh_2026-03_067-070_Gelprin.pdf` (было: `Editorial.pdf` — single-initial автор «Гелприн М.» не распознан)
- MetadataExtractor: v1.3.3; commits: c9d1df5, ede3839
- Верификация: `ls /srv/pdf-extractor/exports/Mh/2026/Mh_2026-03/exports/2026_03_31__19_33_33/articles/`
...
- **Single-initial bylines (RU path — контролируемая поддержка):** Формат «Фамилия И.» (один инициал) поддерживается в ограниченном, контролируемом режиме в `_pick_ru_authors` (MetadataExtractor v1.3.3) через функции `looks_like_single_initial_byline` и `extract_single_initial_byline_prefix` (см. §4.7). Поддержка НЕ является безусловным разрешением single-initial matching во всех языковых и позиционных контекстах. **EN-path намеренно исключён** (см. §4.7, «EN-path exclusion»): empirically обнаружен false-positive на Western-format корреспондентских секциях. STEP C text_block fallback по-прежнему требует 2 инициала — это отдельный, неизменный контракт.
...
- Изменения допускаются **только через выпуск новой версии** (`v_1_3`, `v_2_0` и т.д.)
...
- **v_1_3 — 2026-03-31** — MINOR: добавлен §4.7 «Выбор ru_authors кандидатов»: документированы negative gate (is_running_header) + positive gate (looks_like_author_byline / looks_like_single_initial_byline / extract_single_initial_byline_prefix); зафиксирован EN-path guardrail (намеренное исключение single-initial gate в _pick_en_authors из-за empirical false-positive на Mg_2025-12); §9.1 обновлён: ограничение single-initial больше не универсально верно; acceptance fixture Mh_2026-03 (9 статей, 3 filename errors исправлены)
