# PDF Extractor — BoundaryDetector v_1_4
...
- PDF Extractor — FilenameGenerationPolicy v_1_3 (material_kind usage)
**Previous version:** v_1_3

---

## 1. Цель компонента

BoundaryDetector v_1_4 — детерминированный компонент, который принимает **сырые anchors** (наблюдаемые факты из PDF) и применяет **ArticleStartPolicy v_1_0** для определения:

- страниц начала каждой статьи (article start pages),
- **классификации материала (material_kind: contents/editorial/research/digest/info)** ⭐ info добавлен в v_1_3; digest — детерминированное правило с v_1_3.3,
...
- `article_starts` (array): список найденных начал статей (rich objects).
  ...
  - **`material_kind` (str):** классификация материала
    ...
    - `"digest"` — digest/summary materials (детерминированное правило с v1.3.3)
...
См.: `docs/policies/filename_generation_policy_v_1_3.md` для правил именования по material_kind.
...
**v1.3.2 fix (forward-only window):**

`_has_contents_marker` сканирует только **вперёд** от текущей позиции в anchor-list, не назад. До fix использовалось abs-window, которое могло захватить `contents_marker` из предшествующих позиций в anchor-list. Fix устраняет false Contents classification когда `contents_marker` встречается вне ожидаемой front-matter позиции.

- Commit: `ed0c677` — fix(boundary-detector): forward-only window in `_has_contents_marker` v1.3.2
...
- `_has_digest_title(ru_title)` возвращает False (иначе — digest, см. §3.6.3).
...
BoundaryDetector только классифицирует material_kind и задаёт границы. Генерация filename для Editorial (`{IssuePrefix}_{PPP-PPP}_Editorial.pdf`) выполняется **MetadataVerifier + FilenameGenerationPolicy v_1_3**.
...
BoundaryDetector только классифицирует material_kind и задаёт границы. Генерация filename для Research articles (`{IssuePrefix}_{PPP-PPP}_{FirstAuthorSurname}.pdf`) выполняется **MetadataVerifier + FilenameGenerationPolicy v_1_3**:
...
**Digest (material_kind = "digest"):** ⭐ детерминированное правило добавлено в v_1_3.3

**Назначение:** Детекция дайджест-материалов (обзоры иностранной периодики, тематические дайджесты) по детерминированному правилу на основе `ru_title`.

**Classification rule (v1.3.3, детерминированное):**

Article start классифицируется как **Digest**, если функция `_has_digest_title(ru_title)` возвращает True:
- `ru_title` начинается с «Дайджест» **или** «Digest» (startswith, case-sensitive)
- Проверяется в `_classify_material_kind()` **перед** fallback на editorial

Это правило имеет приоритет над Editorial classification: если `_has_digest_title` True — возвращается `"digest"` независимо от наличия/отсутствия авторов.

**Implementation:** Функция `_has_digest_title(ru_title: str) -> bool` в `agents/boundary_detector/detector.py` v1.3.3.

**Signals object для Digest:**
```json
{
  "primary": "typography_descriptor",
  "font_name": "MyriadPro-BoldIt",
...
**Acceptance validation (Na_2026-03):**
- a07 (стр. 56–94): `ru_title='Дайджест англоязычной отраслевой периодики'` → `material_kind=digest` ✅
- Mg_2025-12 editorial (стр. 5–5): `ru_title='ПРЕДИСЛОВИЕ РЕДАКТОРА'` → не затронут ✅
- Golden test: 28/28 ✅, material classification: 29/29 EXACT MATCH ✅

**Downstream:**

BoundaryDetector классифицирует material_kind = "digest" и задаёт границы. Генерация filename (`{IssuePrefix}_{PPP-PPP}_Digest.pdf`) выполняется **MetadataVerifier + FilenameGenerationPolicy v_1_3** (§3.2).
...
BoundaryDetector классифицирует material_kind = "info" и задаёт границы. Генерация filename (`{IssuePrefix}_{PPP-PPP}_Info.pdf`) выполняется **MetadataVerifier + FilenameGenerationPolicy v_1_3**.
...
BoundaryDetector v_1_4 не делает:
- ❌ разрезание PDF
- ❌ извлечение текста «с нуля»
- ❌ нормализацию/очистку anchors на входе (кроме структурирования per-page)
- ❌ генерацию filename (это MetadataVerifier + FilenameGenerationPolicy v_1_3)
...
Документ **BoundaryDetector v_1_4** является каноническим.

**Предыдущая версия:** BoundaryDetector v_1_3

### Изменения в v_1_4 (2026-04-05)

**Цель:** Документировать v1.3.2 (forward-only window fix в `_has_contents_marker`) и v1.3.3 (детерминированное digest detection rule через `_has_digest_title()`).

**Изменения:**

1. **Header:** FilenameGenerationPolicy dependency обновлён с v_1_2 на v_1_3.

2. **§1 Цель компонента:** Уточнено, что digest — детерминированное правило с v1.3.3 (не «journal-specific» заглушка).

3. **§2.2.1 Поля результата:** `material_kind` enum для `"digest"` обновлён — убрано «если реализовано», добавлено «детерминированное правило с v1.3.3».

4. **§3.6.1 Contents:** Добавлен параграф «v1.3.2 fix (forward-only window)» — документирует замену abs-window на forward-only в `_has_contents_marker`, причину fix'а, commit и validation evidence.

5. **§3.6.2 Editorial:** Добавлено условие: classification rule теперь явно включает `_has_digest_title` = False как необходимое условие для editorial (digest имеет приоритет).

6. **§3.6.3 Digest:** Раздел полностью переписан с «Зарезервировано...» на документированное детерминированное правило:
   - `_has_digest_title(ru_title: str) -> bool` — проверяет startswith «Дайджест»/«Digest»
   - Приоритет перед editorial fallback
   - Acceptance validation: Na_2026-03 a07 (digest ✅), Mg_2025-12 editorial (не затронут ✅)

7. **§3.6.4 Info (Downstream):** Обновлена ссылка FilenameGenerationPolicy v_1_2 → v_1_3.

8. **§5 Границы ответственности:** Заголовок обновлён с v_1_3 на v_1_4; FilenameGenerationPolicy v_1_2 → v_1_3.

**Compatibility:** Additive change. Digest detection rule реализован в v1.3.3 как строгое правило (не heuristic). Consumers, ожидавшие `"editorial"` для digest-материалов с «Дайджест» в ru_title, должны ожидать `"digest"`. Downstream contracts (MetadataVerifier, OutputBuilder, OutputValidator) `digest` уже поддерживают.

**Implementation References:**
- v1.3.2: commit `ed0c677` — fix(boundary-detector): forward-only window in `_has_contents_marker`
- v1.3.3: commit `79466eb` — fix(boundary-detector): detect digest by ru_title prefix
