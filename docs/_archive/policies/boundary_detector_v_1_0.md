# PDF Extractor — BoundaryDetector v1.0

**Project:** PDF Extractor  
**Phase:** 2.4 BoundaryDetector  
**Status:** Canonical  
**Version:** v1.0  
**Depends on:** PDF Extractor — ArticleStartPolicy v1.0  

---

## 1. Цель компонента

BoundaryDetector v1.0 — детерминированный компонент, который принимает **сырые anchors** (наблюдаемые факты из PDF) и применяет **ArticleStartPolicy v1.0** для определения:
...
#### 2.1.2 Требования к anchors (минимальный набор для v1.0)

BoundaryDetector v1.0 требует, чтобы в anchors присутствовали **как минимум**:
...
## 3. Детерминированный алгоритм BoundaryDetector v1.0 (без кода)
...
3) Оценка кандидатов по ArticleStartPolicy v1.0 (signals + constraints + anti-signals) и финальное решение
...
BoundaryDetector v1.0 использует **суперпозицию** источников кандидатов:
...
### 3.3 Стадия 3 — Оценка кандидатов по ArticleStartPolicy v1.0
...
Примечание: BoundaryDetector v1.0 в отчёте signals обязан отражать, **каким способом** был детектирован abstract: marker-based или text-based.

---

#### 3.3.2 Проверка constraints

Constraints из ArticleStartPolicy v1.0:
...
BoundaryDetector v1.0 **не доверяет** любому DOI.
...
BoundaryDetector v1.0 использует фиксированную шкалу (может быть изменена только через v1.1+).
...
- другие anti-signals (если появятся в v1.1+) — только через версионирование.
...
Подтвердить, что BoundaryDetector v1.0:
...
- ✅ реализует ArticleStartPolicy v1.0
- ✅ формирует только границы (start pages)
- ✅ сохраняет аудит signals

BoundaryDetector v1.0 не делает:
- ❌ разрезание PDF
...
Документ **BoundaryDetector v1.0** является каноническим.

Любое изменение:
- только через выпуск новой версии (v1.1+),
